...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			if (TEXTW(m->sel->name) + (m->sel->icon ? m->sel->icw + ICONSPACING : 0) > w) { /* title is bigger than the width of the title rectangle, don't center */
				...
				if (m->sel->icon) drw_pic(drw, (w - TEXTW(m->sel->name)) / 2 + x, (bh - m->sel->ich) / 2, m->sel->icw, m->sel->ich, m->sel->icon);
			}
			if (m->sel->isfloating)
...
main(int argc, char *argv[])
{
	...
	setenv("isdwmrestarting", "0", 0);
	if (strcmp(getenv("isdwmrestarting"), "1")) autostart_exec(); 
	...
	if(restart) {
		setenv("isdwmrestarting", "1", 1);
		execvp(argv[0], argv);
	}
	setenv("isdwmrestarting", "0", 1);
